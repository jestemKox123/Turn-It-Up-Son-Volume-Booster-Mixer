...
(function () {
  ...
  const CHANGELOG = ["4.5.0", "4.4.0", "4.3.1", "4.3.0"];
  window.VBCHANGELOG = {
    list: CHANGELOG,
    max: 3,
...
(function () {
  const DICT = {
    pl: {
      ...
      sec_limiter: "Limiter",
      limiter_h: "Limiter",
      limiter_d: "Pilnuje, żeby dźwięk nie przesterował. Budzi się dopiero wtedy, gdy dźwięk dobija do sufitu, więc na cichym nagraniu nie robi nic. Kiedy to nastąpi, zależy od samego nagrania, od włączonych efektów i od liczby grających kart, a nie od procentu na suwaku. Bez niego głośny bas zaczyna brudzić już trochę powyżej 100%, a od 300% słychać to wyraźnie, bo szczyty ścina wtedy samo nasycenie.",
      limiter_ceil_h: "Sufit głośności",
      limiter_ceil_d: "Poziom, do którego limiter sprowadza dźwięk. Dotyczy całego miksu, razem z basem. 0 dB to maksimum możliwe cyfrowo, czyli najgłośniej i najczyściej, bo limiter ma najmniej pracy. Niżej zostawiasz zapas: ciszej, ale bezpieczniej na słabszym sprzęcie.",
      limiter_bass_h: "Sufit basu",
      limiter_bass_d: "To samo, ale wyłącznie dla dołu (poniżej 150 Hz). Bas ma własny tor pomiarowy, więc możesz go przytrzymać, nie ściągając w dół całego miksu. Zejdź niżej, jeśli bas dudni albo się rozjeżdża.",
      limiter_maxred_h: "Maksymalne dociskanie",
      limiter_maxred_d: "Do ilu decybeli limiter dociska pełną siłą. Powyżej tej wartości nie przestaje pracować, tylko zwalnia mniej więcej o połowę i coraz więcej roboty oddaje nasyceniu, które ścina szczyty. Dlatego miernik obok potrafi pokazać większą liczbę niż ta ustawiona tutaj. Niżej (1 dB) znaczy mocniejszy bas i gęstsze brzmienie, ale wyraźnie więcej zniekształcenia. Wyżej (8 dB) znaczy spokojniejszy, mocniej trzymany bas. Najczyściej jest w okolicach 5 i 6 dB, stąd taka wartość domyślna.",
      limchar_h: "Charakter",
      limchar_d: "Gotowe ustawienia dociskania. Czysto trzyma dźwięk najczystszy i tak zostaje domyślnie. Głośno pozwala limiterowi dociskać krócej, więc gra głośniej, ale szczyty ścina już nasycenie i przy mocnym podbiciu słychać brud. Przy jednej karcie, bez efektów i do 150 procent limiter prawie nie pracuje, więc różnicy nie usłyszysz. Bas, presety z Miksu i kolejne grające karty budzą go dużo wcześniej.",
      limchar_mix_h: "Pokaż charakter w panelu Miks",
      limchar_mix_d: "Te same trzy przyciski pojawią się w okienku, w panelu Miks, pod przełącznikiem pełnego miksera. Dzięki temu zmienisz charakter w trakcie słuchania, bez wchodzenia w ustawienia.",
      mixlim_d: "Ile limiter ma prawo zabrać",
      limchar_clean: "Czysto",
      limchar_mid: "Średnio",
      limchar_loud: "Głośno",
      ...
      limiter_meter_d: "Ile limiter dociska w tej chwili. Zero znaczy, że śpi i w ogóle nie dotyka dźwięku. Paski ruszają się dopiero, gdy któraś karta gra i muzyka dobija do sufitu.",
      limiter_m_bass: "Bas",
      limiter_m_main: "Głośność",
      limiter_idle: "Nic nie gra",
      limiter_tabs: "wspólny dla {n} kart",
      limiter_hq: "Tryb dokładny: wyprzedzenie 5 ms i nasycenie na szczytach",
      limiter_basic: "Tryb zapasowy: bez wyprzedzenia",
    ...
    en: {
      ...
      sec_limiter: "Limiter",
      limiter_h: "Limiter",
      limiter_d: "Keeps the sound from distorting. It only wakes up when the sound reaches the ceiling, so on a quiet recording it does nothing at all. When that happens depends on the recording itself, on the effects you turned on and on how many tabs are playing, not on the percentage on the slider. With it off, loud bass starts getting gritty a little above 100%, and from 300% up you hear it clearly, because then only saturation is shaving the peaks.",
      limiter_ceil_h: "Volume ceiling",
      limiter_ceil_d: "The level the limiter brings the sound down to. It covers the whole mix, bass included. 0 dB is the digital maximum, so loudest and cleanest, because the limiter has the least work to do. Lower leaves headroom: quieter, but safer on weaker gear.",
      limiter_bass_h: "Bass ceiling",
      limiter_bass_d: "The same, but for the low end only (below 150 Hz). Bass has its own detector, so you can hold it down without pulling the whole mix with it. Go lower if the bass booms or breaks up.",
      limiter_maxred_h: "Maximum reduction",
      limiter_maxred_d: "How far the limiter turns things down at full strength. Past that line it does not stop, it slows down by roughly half and hands more and more of the work to saturation, which shaves the peaks. That is why the meter next to it can show a bigger number than the one you set here. Lower (1 dB) means stronger bass and a denser sound, but clearly more distortion. Higher (8 dB) means calmer bass, held down harder. The cleanest spot is around 5 to 6 dB, which is why that is the default.",
      limchar_h: "Character",
      limchar_d: "Ready made clamping settings. Clean keeps the sound at its cleanest and stays the default. Loud lets the limiter clamp for a shorter stretch, so it plays louder, but the peaks are then shaved by saturation and heavy boost gets audibly gritty. With a single tab, no effects and up to 150 percent the limiter barely works, so you will not hear a difference. Bass, Mix presets and more playing tabs wake it up much sooner.",
      limchar_mix_h: "Show character in the Mix panel",
      limchar_mix_d: "The same three buttons show up in the popup, in the Mix panel, under the full mixer switch. That way you change the character while listening, without opening settings.",
      mixlim_d: "How much the limiter may take",
      limchar_clean: "Clean",
      limchar_mid: "Mid",
      limchar_loud: "Loud",
      ...
      limiter_meter_d: "How much the limiter is holding back right now. Zero means it is asleep and not touching the sound at all. The bars only move once a tab is playing and the music reaches the ceiling.",
      limiter_m_bass: "Bass",
      limiter_m_main: "Volume",
      limiter_idle: "Nothing playing",
      limiter_tabs: "shared across {n} tabs",
      limiter_hq: "Precise mode: 5 ms lookahead and saturation on the peaks",
      limiter_basic: "Fallback mode: no lookahead",
